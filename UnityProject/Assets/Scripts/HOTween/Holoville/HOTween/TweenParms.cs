		...
		internal void InitializeObject(Tweener p_tweenObj, object p_target)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (aBSTweenPlugin != null)
				{
					...
					{
						string text = aBSTweenPlugin2.GetType().ToString();
						TweenWarning.Log(string.Concat(text.Substring(text.LastIndexOf('.') + 1), " : Invalid target (", p_target, "). The tween for this property will not be created."));
						continue;
					}
