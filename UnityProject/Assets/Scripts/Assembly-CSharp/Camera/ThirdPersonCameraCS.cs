...
public class ThirdPersonCameraCS : MonoBehaviour
{
	...
	public static bool draggingByHand;
	...
	public void ApplyManual(Transform dummyTarget, Vector3 dummyCenter)
	{
		if (!controller)
		{
			return;
		}
		Vector3 position = _target.position + centerOffset;
		Vector3 vector = _target.position + headOffset;
		DebugDrawStuff();
		...
		if (snap)
		{
			if ((double)AngleDistance(y2, y) < 3.0)
			{
				snap = false;
			}
		...
		cameraTransform.Translate(quaternion * Vector3.back * distance);
	...
	private void LateUpdate()
	{
		...
		{
			ApplyManual(base.transform, Vector3.zero);
		}
	}
	...
	private void Cut(Transform dummyTarget, Vector3 dummyCenter)
	{
		...
		snap = true;
		Apply(dummyTarget, Vector3.zero);
		heightSmoothLag = num;
		snapMaxSpeed = num2;
		snapSmoothLag = num3;
	}
