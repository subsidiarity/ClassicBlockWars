...
public class UICamera : MonoBehaviour
{
	...
	public static bool Raycast(Vector3 inPos, out RaycastHit hit)
	{
		for (int i = 0; i < list.size; i++)
		{
			...
			Vector3 point = currentCamera.ScreenToViewportPoint(inPos);
			if (float.IsNaN(point.x) || float.IsNaN(point.y) || point.x < 0f || point.x > 1f || point.y < 0f || point.y > 1f)
			{
				continue;
			}
			...
			{
				if (Physics.Raycast(ray, out hit, enter, layerMask))
				{
			...
			else if (uICamera.eventType == EventType.Unity2D && m2DPlane.Raycast(ray, out enter))
			{
				Collider2D collider2D = Physics2D.OverlapPoint(ray.GetPoint(enter), layerMask);
				if ((bool)collider2D)
				{
					hit = lastHit;
					hit.point = point;
